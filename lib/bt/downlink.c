...
LOG_MODULE_REGISTER(downlink_gatt, LOG_LEVEL_DBG);
...
static int write_downlink_characteristic(void *conn, const void *data, size_t length)
{
    ...
    // LOG_DBG("Writing %d bytes to handle %d", length, downlink_handle);

    int err =
        bt_gatt_write_without_response_cb(conn, downlink_handle, data, length, false, NULL, NULL);
    if (err)
    {
...
{
    LOG_ERR(">>>> Downlink data available");
