...
LOG_MODULE_REGISTER(uplink_gatt, LOG_LEVEL_DBG);

#define UPLINK_WINDOW_SIZE 1
                                ...
                                uint16_t length)
{
    ...
    if (pouch_gatt_packetizer_is_fin(data, length))
    {
        LOG_DBG("Received end from node");

        if (node->uplink)
        {
            LOG_WRN("Node ended uplink prematurely");
    ...
    int err = pouch_gatt_receiver_receive_data(node->uplink_receiver, data, length);
    if (err)
    {
